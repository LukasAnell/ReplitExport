...
public class MyStory
{
  ...
  public void start() {
    System.out.println("You are walking to work when you hear a strange sound coming from your right. You see an alleyway with a few abandoned storefronts.");
    wait(500);
    System.out.println("Will you continue to work or peel off into the alley to investigate the sound?");
  ...
  private void work() {
    System.out.println("You continue to work, the sound getting quieter. You continue walking past storefronts with \nexotic items, whistling an upbeat tune. You arrive at work, where there is a group of men dressed in black garbs, holding sharp objects.");
    wait(500);
    System.out.println("Will you walk towards them or run away?");
    ...
    if(input.equals("walk towards them") || input.equals("walk")) {
      ...
      wait(500);
  System.out.println("You try to run away, but one of the men throws an axe at your head and you die. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void kidnapping() {
    System.out.println("You look around, taking in your surroundings. You must have been out for a while, because the faint beam of light coming from the ceiling is now orange. Confused, you get up, but are hit by a wave of dizziness and nausea.");
    wait(500);
    System.out.println("Will you go back to sleep or explore the room?");
    ...
    if(input.equals("go back to sleep") || input.equals("sleep") || input.equals("go back")) {
      wait(500);
      System.out.println("You go back to sleep, having wild dreams of living in the upper class, partying and having fun all the time. Unfortunately, your nausea was caused by a deadly drug, so you die in your sleep. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void box() {
    System.out.println("You get up, fighting back the nausea. As you struggle to stand, a shiny box catches your eye. You hobble over to it, using the wall as support, and open it. Inside, there are three items: a pill, a syringe, and a knife.");
    wait(500);
    System.out.println("Which item will you take: the pill, the syringe, or the knife?");
    ...
    if(input.equals("the pill") || input.equals("pill")) {
      wait(500);
      System.out.println("You pick up the pill, but don't have a place to put it. You being a smart individual, decide to put it in your mouth. Unfortunately, the pill is easily broken and releases a deadly toxin into your mouth, killing you instantly. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void wood() {
    System.out.println("You grab the knife, swinging it around, and head back to the chair. Approaching the chair, you \nare intrigued by a an irregularity in the grain, so you start picking at it. As soon as you begin, an alarm starts blaring.");
    wait(500);
    System.out.println("Will you dig deeper into the wood or will you look up?");
  ...
  private void food() {
    ...
    wait(500);
    System.out.println("Will you read the note or eat the food?");
  ...
  private void escape() {
    System.out.println("The text on the note says \"You are currently in the Upper Mons District, in the largest estate on Mars. It is packed with the highest technology security system, so do not try to escape. You have 24 hours to live your life to the fullest before we come and kill you for intruding on us.\"");
    wait(500);
    System.out.println("Will you try to escape or live your life?");
    ...
    if(input.equals("live your life") || input.equals("live my life") || input.equals("live")) {
      wait(500);
      System.out.println("You relax, eating whatever you want, sleeping, gaming, and watching the view. You stay in the house until the men come to get you the next day. You are taken away in a limousine to an executin center. There you are strapped into an electric chair and zapped to death. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void intersection() {
    ...
    wait(500);
    System.out.println("Will you approach the men or continue picking at the chair?");
  ...
  private void sphere() {
    ...
    wait(500);
    System.out.println("Will you open the sphere or throw it?");
    ...
    if(input.equals("throw it") || input.equals("throw") || input.equals("throw the sphere")) {
      wait(500);
      System.out.println("You chuck the sphere across the room, using all of your strength to get a lot of impact force. It opens up, spilling out a purple gas, which then ignites. The end");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void frequency() {
    ...
    wait(500);
    System.out.println("Approach the sound or try to wake up one of the men?");
    ...
    if(input.equals("approach") || input.equals("approach the sound") || input.equals("sound")) {
      wait(500);
      System.out.println("You approach the sound in a cautioned stance, ready to run away at the slightest bit of danger.  As soon as you cross the line of men, the sound explodes into your ears. You try to run away, but you are stuck in place. As the sound drones on, the vibrations become unbearable and you die. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void investigate() {
    System.out.println("You are led down an alleyway, where you see an abandoned storefront.");
    wait(500);
    System.out.println("Will you walk in or walk away?");
    String input = UserInput.getValidInput("walk in", "walk away");
    if(input.equals("walk away")) {
      wait(500);
      System.out.println("You try to walk away, but a piano is dropped onto you, effectively killing you. The end.");
    }
    else {
      wait(500);
      stuck();
  ...
  private void stuck() {
    System.out.println("You stroll into the shop, the door roughly closing behind you. You find yourself in a cylindrical room, with stone walls and a faint beam of light coming from above.");
    wait(500);
    System.out.println("You look behind you, but the door has been replaced by a smooth wall of stone. Walking around, you notice scratches on the walls, as if they were made by a creature with large claws. You continue to look around, and notice a shiny box. Looking inside, you see a pill, a syringe, and a knife.");
    System.out.println("Which item will you take: the pill, the syringe, or the knife?");
    String input = UserInput.getValidInput("the pill", "pill", "the syringe", "syringe", "the knife", "knife");
    if(input.equals("the pill") || input.equals("pill")) {
      wait(500);
  ...
  private void toiletPaper() {
    ...
    System.out.println("Your attempt fails, so now there is 6 guards looking directly at you.");
    System.out.println("Will you jump down and fight them or try to run away?");
    ...
    if(input.equals("run away") || input.equals("run")) {
      wait(500);
      System.out.println("You try to run away, but the guards activate a lockdown of the facility, so guns come out of the walls. You try to dodge the bullets but you get hit and die. The end.");
      timesRestarted++;
      wait(500);
      start();
    }
  ...
  private void fight() {
    System.out.println("You jump down to fight the guards, confident in your fists. As you land, the guards start running towards you. You ready yourself for the onslaught of attacks coming, and start waving your fists everywhere.");
    System.out.println("You come out of a pile of guards on the floor, having successuflly gotten past all of them, and clutch your chest and arms. You are hurt, and took " + ((int) Math.random() * 10 + 1) + " punches.");
  }
